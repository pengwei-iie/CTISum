...
def train_model(args):
    ...
    if args.mode == 'train':
        data_paths['train'] = "{:s}/{:s}_{:d}.jsonl".format(args.raw_path, 'train', p_ct)
        data_paths['test'] = "{:s}/{:s}_{:d}.jsonl".format(args.raw_path, 'test', p_ct)
    else:
        data_paths['val'] = "{:s}/{:s}_{:d}.jsonl".format(args.raw_path, 'valid', p_ct)
    ...
    valid_set = datasets.datasets['test']
    ...
    val_metric = [ValidMetric(save_path=args.save_path, data=read_jsonl(data_paths['test']))]
...
def test_model(args):
    ...
    # test_set = datasets.datasets['val']
    ...
    for cur_model in models:
        ...
        # test_metric = MatchRougeMetric(data=read_jsonl(data_paths['val']), dec_path=dec_path,
        #                                ref_path=ref_path, n_total=len(test_set))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', required=True,
                        help='root of the model', type=str)
    # example for gpus input: '0,1,2,3' or 'cpu'
    parser.add_argument('--gpus', required=True,
                        help='available gpus for training(separated by commas)', type=str)
    # parser.add_argument('--gpus', default='cpu',
    #                     help='available gpus for training(separated by commas)', type=str)
    ...
    parser.add_argument('--candidate_num', default=4,
