...
def get_candidates(original_data, sent_ids, type, tokenizer, cls, sep_id, idx):
    ...
    type_path = join(temp_path, 'processed')
    processed_path = join(type_path, type)
    print('processed_path',processed_path)
    with open(join(processed_path, type + '_{}.json'.format(idx)), 'w') as f:
...
def get_candidates_mp(args):
    ...
    for type in ['valid', 'test', 'train']:
        for data_f in glob.glob(pjoin(args.data_path, type + '_[0-9]*.jsonl')):
            data_name = data_f.split('/')[-1]
            for index_f in glob.glob(pjoin(args.index_path, type + '_[0-9]*.jsonl')):
                index_name = index_f.split('/')[-1]
                if (data_name == index_name):
                    original_data = load_jsonl(data_f)
                    sent_ids = load_jsonl(index_f)
                    ...
                    with mp.Pool() as pool:
                        partial_get_candidates = partial(get_candidates, original_data, sent_ids, type, tokenizer, cls, sep_id)
                        list(pool.imap_unordered(partial_get_candidates, range(n_files), chunksize=64))
                    ...
                    for i in range(n_files):
                        with open(join(processed_path, type + '_{}.json'.format(i))) as f:
                            data = json.loads(f.read())
                        with open(join(args.write_path, data_name),'a') as f:
