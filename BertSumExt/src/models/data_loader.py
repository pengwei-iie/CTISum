...
def load_dataset(args, corpus_type, shuffle):
    ...
    pts = sorted(glob.glob(args.bert_data_path + '/' + corpus_type + '_[0-9]*.bert.pt'))
    if pts:
        # print('shini')
        if (shuffle):
            random.shuffle(pts)
        ...
        pt = args.bert_data_path + '/' + corpus_type + '_0.bert.pt'
        yield _lazy_dataset_loader(pt, corpus_type)
...
class Dataloader(object):
    ...
    def _next_dataset_iterator(self, dataset_iter):
        # 这个函数的设计可能是为了在处理大型数据集时，及时释放之前数据集的内存，以避免内存占用过高。
        # 函数中的垃圾回收和释放内存的步骤可能是为了确保在切换到新数据集时及时释放旧数据集的内存。
        try:
